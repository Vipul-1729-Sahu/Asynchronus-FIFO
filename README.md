...
This project implements an **Asynchronous FIFO** in Verilog HDL. The FIFO is used to safely transfer multi-bit data between two independent clock domains: a write clock domain and a read clock domain.

In digital systems, directly transferring multi-bit data between two unrelated clocks can cause metastability and data corruption. This design solves that problem using a dual-clock FIFO architecture with Gray-coded pointers and two-flop synchronizers.
...
* Binary read/write pointers for address generation
...
An asynchronous FIFO is required when data needs to be transferred between two modules operating on different clocks.

For example:

* Data producer working on one clock
* Data consumer working on another clock
* Different clock frequencies
* Independent clock phases
* CDC-safe buffering in SoC designs

Instead of directly passing a multi-bit bus across clock domains, the FIFO stores data in memory and only synchronizes the control pointers. This makes the data transfer safer and more reliable.

## Architecture

The design is divided into small modules:

```text
Async_FIFO
├── fifo_mem       - Dual-clock FIFO memory
├── sync_pointer   - Two-flop synchronizer for Gray pointers
├── wr_brain       - Write pointer, Gray conversion, full logic
└── rd_brain       - Read pointer, Gray conversion, empty logic
```
...
Only Gray-coded pointers are synchronized because Gray code changes only one bit at a time, reducing the risk of incorrect pointer sampling.
...
In binary pointer logic, full can be understood as the write pointer being one complete FIFO depth ahead of the read pointer. However, since the comparison is done in Gray code, the top two MSBs need to be inverted.
...
The user only provides write/read requests. Internally, the FIFO controller generates the memory addresses using read and write pointers.
...
7. Extra write when FIFO is full
8. Extra read when FIFO is empty
...
```markdown
![Asynchronous FIFO Simulation Waveform](waveform/async_fifo_simulation.png)
```
